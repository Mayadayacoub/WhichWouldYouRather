...
const trophyColor = ["red", "purple", "orange"];
...
export class Leaderboard extends Component {
  ...
  render() {
    ...
    return (
      <Fragment>
        {leaderboardData.map((user, idd) => (
          <Segment.Group key={user.id}>
            <Label corner="left" color={trophyColor[idd]}>
              <Icon loading name="star outline" />
            </Label>
            <Grid divided padded>
              <Grid.Row>
                <Grid.Column width={9} verticalAlign="middle">
                  <Image src={user.avatarURL} rounded floated="left" />
                </Grid.Column>
                <Grid.Column width={8}>
